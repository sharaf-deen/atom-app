...
begin
  ...
  select i.*, o.status as supplier_order_status
  into v_item
  from public.store_supplier_order_items i
  join public.store_supplier_orders o on o.id = i.supplier_order_id
  where i.id = _item_id
  for update of i, o;
  ...
  update public.store_supplier_order_items
  set
  ...
  if v_delta > 0 and v_item.product_id is not null then
    ...
    set inventory_qty = inventory_qty + v_delta
    where id = v_item.product_id
    returning inventory_qty into v_inventory;
  elsif v_item.product_id is not null then
    select p.inventory_qty into v_inventory
  ...
  select
    count(*)::integer,
    count(*) filter (where line_status = 'received')::integer,
    count(*) filter (where received_qty > 0)::integer
  into v_total_items, v_received_items, v_touched_items
  from public.store_supplier_order_items
  where supplier_order_id = v_item.supplier_order_id
  ...
  update public.store_supplier_orders
  set
    status = v_order_status,
    ordered_at = coalesce(ordered_at, timezone('utc', now())),
    received_at = case when v_order_status = 'received' then timezone('utc', now()) else null end,
    updated_by = v_uid,
    updated_at = timezone('utc', now())
  where id = v_item.supplier_order_id;
