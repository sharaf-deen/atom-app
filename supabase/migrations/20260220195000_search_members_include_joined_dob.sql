-- search_members v6: include created_at + date_of_birth for admin Members UI (Joined/Age).
-- Recreates search_members(q, status, page, page_size) with same behavior.

-- Param name fix v2:
-- We must DROP then CREATE to rename input parameters.
-- This recreates search_members(q, status, page, page_size) with the same behavior as v5.
