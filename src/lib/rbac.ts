...
const APP_NAV_BY_ROLE: MenuByRole = {
  member: [
    ...
    { label: 'Private Coaching', href: '/private-coaching', icon: 'user-cog' },
...
export function canAccessCoachTrainingPrograms(role: Role | null | undefined) {
  return hasAnyRole(role, ['assistant_coach', 'coach', 'head_coach', 'super_admin'])
}
...
export function canAccessCoachStaffAttendance(role: Role | null | undefined) {
  return hasAnyRole(role, ['assistant_coach', 'coach', 'head_coach', 'super_admin'])
}
...
export function canAccessScheduleOperations(role: Role | null | undefined) {
  return hasAnyRole(role, ['assistant_coach', 'coach', 'head_coach', 'super_admin'])
}
...
export function canAccessStoreFunding(role: Role | null | undefined) {
  return hasAnyRole(role, STORE_FUNDING_ROLES)
}

export function canManageStoreFunding(role: Role | null | undefined) {
  return hasAnyRole(role, SUPER_ADMIN_ROLES)
}
...
const CAPABILITY_BLUEPRINTS: CapabilityBlueprint[] = [
  ...
  },
  {
    ...
    description: 'Open the unified family onboarding, Visitor conversion and Family Account workspace.',
  ...
  },
  {
    ...
    description: 'Open store business overview for admin and super admin.',
  ...
  },
  {
    key: 'store_funding',
    category: 'Store',
    label: 'Store external funding',
    description: 'Review external funding loans and repayments for Store cash visibility. Admin is read-only; super admin can manage entries.',
    href: '/admin/store/funding',
    check: (role) => canAccessStoreFunding(role),
  },
  {
    key: 'store_admin',
    category: 'Store',
