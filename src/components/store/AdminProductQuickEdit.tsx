...
export default function AdminProductQuickEdit({
  ...
  async function deleteProduct() {
    ...
    const ok = window.confirm('Delete this product? This is blocked if the product is already linked to store history.')
    if (!ok) return
    ...
    <>
      <div className="grid gap-3 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-[120px_140px_auto_auto_auto] xl:items-end">
          ...
          <Button onClick={save} disabled={!dirty || loading} loading={loading} loadingText="Saving…" className="h-[40px]">
            Save
          </Button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button type="button" variant="outline" onClick={deleteProduct} loading={deleteBusy} loadingText="Deleting…">
            Delete product
          </Button>
        </div>
      </div>

      <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Edit product" className="w-[min(96vw,44rem)]">
        <StoreProductForm
          product={{
            id,
            category,
