...
const listStoreProductsCached = unstable_cache(
  async (
    ...
    let qry = supa
      .from('store_products')
      .select('id, category, name, color, size, price_cents, currency, inventory_qty, is_active, image_path, created_at')
    ...
    const bucket = supa.storage.from('store-product-images')
    const withImages = sliced.map((row) => ({
      ...row,
      image_url: row?.image_path ? bucket.getPublicUrl(row.image_path).data.publicUrl : null,
    }))
    return { items: withImages as any, hasMore }
  },
  ['store_products_v3_images'],
                  ...
                  <CardContent className="py-4 space-y-3">
                    {p.image_url ? (
                      <img
                        src={p.image_url}
                        alt={p.name}
                        className="h-48 w-full rounded-xl border border-[hsl(var(--border))] object-cover"
                      />
                    ) : (
                      <div className="flex h-48 w-full items-center justify-center rounded-xl border border-dashed border-[hsl(var(--border))] bg-[hsl(var(--card))] text-xs uppercase tracking-wide text-[hsl(var(--muted))]">
                        No photo
                      </div>
                    )}
