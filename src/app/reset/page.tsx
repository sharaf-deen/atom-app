...
function strengthColor(score: number) {
  return ['bg-red-500', 'bg-red-500', 'bg-yellow-500', 'bg-amber-500', 'bg-green-500', 'bg-green-600'][Math.min(5, Math.max(0, score))]
}
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        // 1) Most common after Supabase redirect: tokens in hash (#access_token=...&refresh_token=...)
        if (params.access_token && params.refresh_token) {
          ...
          // Clean URL (remove tokens in hash)
        ...
        if (params.type) {
          const otpType = (params.type || 'recovery') as 'recovery' | 'magiclink' | 'invite' | 'signup' | 'email_change'
        ...
        // 3) PKCE code flow (common with Supabase resetPasswordForEmail)
        if (params.code) {
        ...
        // 4) Last resort: session already present
        const {
          data: { session },
  ...
  return (
    <main className="max-w-sm mx-auto p-6 space-y-4">
      ...
      {stage === 'exchanging' && <div className="p-3 border rounded bg-white">{msg}</div>}

      {stage === 'error' && (
        <div className="p-3 border rounded bg-red-50 border-red-300 text-sm space-y-2">
          <div>{msg}</div>
          <div className="text-gray-600">Try requesting a new reset link from the login page.</div>
        ...
        <form onSubmit={submitNewPassword} className="space-y-3 border rounded p-4 bg-white">
          ...
          <button
            ...
            className={`w-full border rounded px-4 py-2 ${
              canSubmit ? 'bg-gray-50 hover:bg-gray-100' : 'bg-gray-100 opacity-60 cursor-not-allowed'
            }`}
      ...
      {stage === 'done' && <div className="p-3 border rounded bg-green-50 border-green-300 text-sm">{msg}</div>}
    </main>
  )
}
