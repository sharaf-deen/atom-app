...
export type PublishedTrainingProgram = {
  id: string
  title: string
  target_group: string
  start_date: string
...
export type ScheduleSessionTrainingProgramAssignment = {
  id: string
  training_session_id: string
  program_id: string
  program_title_snapshot: string
  target_group_snapshot: string
  program_start_date_snapshot: string
  program_end_date_snapshot: string
  assigned_at: string
}
...
export default async function ScheduleTrainingSessionsPage() {
  ...
  const [sessionResult, programsResult] = await Promise.all([
    ...
    supabase
      ...
      .select('id,title,target_group,start_date,end_date')
      .eq('status', 'published')
      .lte('start_date', previewUntil)
      .gte('end_date', today)
    ...
    if (visibleSessionIds.length > 0) {
      const [activeAssignmentsResult, activeProgramsResult] = await Promise.all([
        ...
        supabase
          ...
          .select(
            'id,training_session_id,program_id,program_title_snapshot,target_group_snapshot,program_start_date_snapshot,program_end_date_snapshot,assigned_at',
          )
          .eq('is_active', true)
          .in('training_session_id', visibleSessionIds),
      ])
      ...
      else assignments = (activeAssignmentsResult.data ?? []) as ScheduleSessionCoachAssignment[]
  ...
  return (
    <main>
      <PageHeader
        ...
        subtitle={
          personalView
            ? 'Your upcoming structured ATOM coaching assignments, planned program and session context.'
            : 'Dated academy sessions with staff assignments and planned Training Programs.'
        }
      />
      <Section className="max-w-6xl space-y-4">
        ...
        {personalView ? (
          <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-950">
            This view shows only sessions where you are currently assigned as Primary Coach or Assistant Coach. The linked Training Program is read-only for Coach and Assistant Coach.
          </div>
        ) : (
          <>
            <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-950">
