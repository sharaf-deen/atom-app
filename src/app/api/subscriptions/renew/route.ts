...
function dateOnlyCairo(d = new Date()) {
  try {
...
export async function POST(req: Request) {
  try {
    ...
    // Renewal policy:
// - We allow an existing active subscription.
// - For time plans, the new subscription starts AFTER the latest active time plan ends (if any).
const todayCairo = dateOnlyCairo()

let maxActiveTimeEnd: string | null = null
try {
  const { data: activeTime, error: atErr } = await admin
    .from('subscriptions')
  ...
  if (!atErr) {
    for (const s of activeTime ?? []) {
      const end = (s as any)?.end_date as string | null
      if (!end || !isISODateOnly(end)) continue
      // Consider only subs that haven't ended yet (Egypt day)
      if (end < todayCairo) continue
      if (!maxActiveTimeEnd || end > maxActiveTimeEnd) maxActiveTimeEnd = end
    }
  }
} catch {
  // ignore
...
// 4) Build insert payload
    ...
    if (plan === 'sessions') {
      ...
      const requestedStart = isISODateOnly(body?.start_date) ? String(body.start_date) : todayCairo
      ...
      // Time plans: compute start_date for renewal.
// - If an active time subscription exists, chain after its end_date.
// - Otherwise use provided start_date if valid, else today (Egypt).
