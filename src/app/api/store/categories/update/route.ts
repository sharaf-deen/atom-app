...
export async function PATCH(req: Request) {
  try {
    ...
    const oldKey = normalizeStoreCategoryKey(body?.old_key || body?.oldKey)
    const nextKey = normalizeStoreCategoryKey(body?.key)
    const label = String(body?.label || '').trim().slice(0, 80)
    const sortOrder = Number.isFinite(Number(body?.sort_order)) ? Math.trunc(Number(body.sort_order)) : 0
    const isActive = body?.is_active === undefined ? true : Boolean(body.is_active)

    if (!oldKey || !nextKey) {
      return noStore(NextResponse.json({ ok: false, error: 'INVALID_KEY', details: 'A valid key is required.' }, { status: 400 }))
    }
    if (!label) {
