...
export async function POST(req: Request) {
  try {
    ...
    // Sanitize kind pour respecter la contrainte SQL
    ...
    // Rappel: ta table a NOT NULL sur member_id → on met member_id = user_id
    const rows = Array.from(recipientIds).map((uid) => ({
      user_id: uid,        // destinataire
      member_id: uid,      // satisfait NOT NULL
      title,
      body: msg,
    ...
    for (let i = 0; i < rows.length; i += CHUNK) {
      ...
      const { error } = await supa.from('notifications').insert(slice)
      if (error) {
