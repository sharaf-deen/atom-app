...
async function uploadAttachment(admin: ReturnType<typeof createSupabaseAdminClient>, file: File, expenseDate: string) {
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const expenseDate = safeStr(form.get('expense_date'))
    const category = safeStr(form.get('category'))
    ...
    if (!isDateOnly(expenseDate)) return json(400, { ok: false, error: 'INVALID_DATE' })
    if (!CATEGORIES.has(category)) return json(400, { ok: false, error: 'INVALID_CATEGORY' })
    ...
    if (attachment && typeof (attachment as any)?.arrayBuffer === 'function') {
      ...
      if (file.size > 0) {
        const uploaded = await uploadAttachment(admin, file, expenseDate)
    ...
    if (updateErr) {
      if (newPath) await admin.storage.from(STORE_EXPENSE_BUCKET).remove([newPath]).catch(() => null)
    ...
    if (newPath && before.attachment_path && before.attachment_path !== newPath) {
      await admin.storage.from(STORE_EXPENSE_BUCKET).remove([before.attachment_path]).catch(() => null)
    }
