...
function parseProofFilter(value: string | null): ProofFilter {
  return value === 'with_proof' || value === 'missing_proof' ? value : 'all'
}

function parseSupplierLinkFilter(value: string | null): SupplierLinkFilter {
...
export async function GET(req: Request) {
  ...
  const supplierLinkFilter = supplierOrderId !== 'all' ? 'linked' : parseSupplierLinkFilter(url.searchParams.get('supplier_link'))
  const proofFilter = parseProofFilter(url.searchParams.get('proof'))
  const q = sanitizeSearch(url.searchParams.get('q') || '')

  const admin = createSupabaseAdminClient()
  let query = admin
    .from('store_expenses')
    .select('id,expense_date,category,title,amount_cents,currency,payment_method,supplier_order_id,vendor_name,note,attachment_path,attachment_filename,created_at,updated_at')
    .is('deleted_at', null)
  ...
  else if (supplierLinkFilter === 'linked') query = query.not('supplier_order_id', 'is', null)
  else if (supplierLinkFilter === 'unlinked') query = query.is('supplier_order_id', null)
  if (proofFilter === 'with_proof') query = query.not('attachment_path', 'is', null)
  if (proofFilter === 'missing_proof') query = query.is('attachment_path', null)
  if (q) {
    const like = `%${q}%`
  ...
  const body = rows.map((row) => csvLine([
    ...
    row.currency || 'EGP',
    row.note || '',
    row.attachment_path ? 'With proof' : 'Missing proof',
