...
/** Password strength utils (same as before) */
...
/** Helpers to read either query or hash */
function getURLParams(url: string) {
  ...
  return {
    code: get('code'),
    token: get('token') || get('token_hash'), // supabase sometimes uses token, sometimes token_hash
    type: get('type'), // invite | magiclink | recovery | signup | email_change
  };
}
...
export default function WelcomePage() {
  ...
  const [msg, setMsg] = useState('Finalizing your activation link…');
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const { code, token, type } = getURLParams(window.location.href);

        if (code) {
          // Flow PKCE (OAuth/code): use exchangeCodeForSession
          const { error } = await supabase.auth.exchangeCodeForSession(window.location.href);
          if (error) throw error;
        } else if (token && type) {
          // Flow Email Token: verifyOtp with token_hash + type
          // type can be: invite | magiclink | recovery | signup | email_change
          ...
          throw new Error('Activation link is missing required parameters (code or token_hash).');
        }

        // If we get here, we have a session: fetch user email
        const { data: { user } } = await supabase.auth.getUser();
        setEmail(user?.email ?? null);
        setStage('form');
      } catch (e: any) {
        setMsg(`Activation link error: ${e?.message || e}`);
        setStage('error');
      }
    };
    run();
  ...
  async function submitNewPassword(e: React.FormEvent) {
    ...
    try {
      ...
      setStage('done');
      setMsg('✅ Password set. Redirecting to your profile…');
  ...
  return (
    <main className="max-w-sm mx-auto p-6 space-y-4">
      <h1 className="text-2xl font-bold">Welcome to Atom Jiu-Jitsu HQ</h1>
      ...
      {stage === 'error' && (
        <div className="p-3 border rounded bg-red-50 border-red-300 text-sm">
          ...
          <span className="text-gray-600">
            Try opening the invitation link again or contact the academy.
          </span>
        </div>
      )}
      ...
      {stage === 'form' && (
        <form onSubmit={submitNewPassword} className="space-y-3 border rounded p-4 bg-white">
          <p className="text-sm text-gray-700">
            {email ? <>Hi <b>{email}</b>,</> : 'Hi,'} please set your password to activate your account.
          </p>
